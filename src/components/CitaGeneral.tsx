  ...
  const onSubmit: SubmitHandler<AppointmentFormData> = async (data) => {
    try {
      ...
      const response = await fetch(`http://localhost:3000/appointments/${id}`, {
        ...
        body: JSON.stringify({
          ...data,
          reference, // Incluimos la referencia si existe
        }),
      });

      if (response.ok) {
        console.log("Cita actualizada correctamente");
        navigate("/General/dashboard"); // Redirigir al dashboard si la actualización es exitosa
      } else {
    ...
    <div className="bg-[#f0f0f5] min-h-screen p-0">
      ...
      >
        <h2 className="text-2xl font-bold text-[#9588d0]">
          Cita - {patient.first_name} {patient.first_last_name}
        </h2>
      </div>
